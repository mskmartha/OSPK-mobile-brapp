...
class BaseBindableViewModelTest : BaseTest() {
    ...
    private val TEST_ALT_DATA = "alt_data"
    ...
    data class TestAltBindableViewModel(val data: String) : BaseBindableViewModel() {
        private val TEST_LAYOUT = 1226

        override fun getItemFactory(): (BaseBindableViewModel) -> BindableItem<ViewDataBinding> {
            return { vm -> ViewModelItem(vm as TestAltBindableViewModel, TEST_LAYOUT) }
        }
    }
    ...
    fun viewModelItem_shouldNotBeSameAs_anotherViewModelItemOfAnotherType() {
        val tvm = TestBindableViewModel(TEST_DATA)
        val vmi = tvm.getItem(0)
        val tavm = TestAltBindableViewModel(TEST_ALT_DATA)
        val tavmi = tavm.getItem(0)
        assertThat(vmi.isSameAs(tavmi)).isFalse()
    }
