...
interface LiveDataHelper {
    ...
    fun <T> LiveData<T>.set(value: T) = (this as? MutableLiveData<T>)?.setValue(value) ?: run { Timber.w("[post] unable to setValue for $this") }
    /** Helper function to avoid needing downcast declarations for public MutableLiveData */
    fun <T> LiveData<T>.postValue(value: T) = (this as? MutableLiveData<T>)?.postValue(value) ?: run { Timber.w("[post] unable to postValue for $this") }
}
